...
const Kayp = () => {
  ...
  useEffect(() => {
    ...
    fetchEvents();
  }, [address]); // Dependency array includes address to refetch when account changes

  return (
    <>
      <Text>
        {ownedTokenIds.length > 0 ? (
          <p>Your addrress {address} owns token IDs: {ownedTokenIds.join(', ')}</p>
        ) : (
          <p>Your addrress {address} owns no tokens.</p>
        )}
      </Text>
